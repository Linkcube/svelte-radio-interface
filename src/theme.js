import { writable } from 'svelte/store';

export const primary = writable("#3f51b5");

export const highlight = writable("#3c8bff");

export const background = writable("white");

export const text_on_color = writable("white");

export const base_text_color = writable("black");

export const sub_text_color = writable("gray");

export const hover_color = writable("lightgray");

export const input_hover_color = writable("yellow");

export const primary_light = writable("#EBF6FA");